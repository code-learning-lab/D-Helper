...
if __name__ == "__main__":
	...
	for epoch in range(EPOCHS):
		...
		overlearn=0
		while i < len(train_x):
			batch = get_batch(train_x, train_y, i-sum, BATCH_SIZE)
			...
			if(overlearn<5):
				...
				confidence_scores=confidence_scores.cpu().detach().numpy()

				labels=train_labels.cpu().detach().numpy()
				ordered = get_noise_indices(s=labels,
											psx=confidence_scores,
											sorted_index_method='normalized_margin',
											)

			if (len(ordered) == 0):
				overlearn = overlearn + 1;
			if(len(ordered)!=0):
				ordered.sort()
				count = len(ordered)
				sum = sum + count
				for index in sorted(ordered):
					del(train_x[i+index-BATCH_SIZE-z])
					del(train_y[i+index-BATCH_SIZE-z])
					z=z+1
